...
class tribune{
	...
	var $display;
	var $i=-1;
	var $limit_com_start = 0;
	var $limit_show_com = 20;
...
$this->infos .= "<span class='tktr_info'>$this->i $message </span>";
	...
	public function __construct($debugstate=0) {
...
try {
	...
	public function comform($id_tribune){
		$this->display.="
			<div class='tktr_comform' id='tktr_add_comm_".$id_tribune."'>
...
";
	...
	public function com_list($id_tribune){
		...
		$this->init_pdo();
		$limit_com_start = $this->limit_com_start;
		$limit_show_com = $this->limit_show_com ;
		
		$nb_coms = $this->pdo->query("SELECT count( id ) from tk_tribune where id_tribune = '$id_tribune'");
		$nb_coms = $nb_coms->fetchAll();
		$nb_coms = $nb_coms[0][0];
		$query = $this->pdo->query("SELECT * from tk_tribune where id_tribune = '$id_tribune' AND class = 'show' ORDER BY datetime DESC LIMIT $limit_com_start , $limit_show_com") ;
		//		print_r($query);
	//	$nb_coms = $query->rowCount();
		if($query->rowCount()==0){
			...
			$this->comform($id_tribune);
		//	echo"start $limit_com_start , + $limit_show_com";
		...
		$this->display .= "<a href='#tktr_add_comm_".$id_tribune."'><span class='tktr_com_foot'> Vous aussi participez!</span></a>";
...
.tktr_comform input:active,.tktr_comform input:focus {
	background:#F8FFD1;
	color: #3D3C21;
}
.tktr_comform textarea{
...
.tktr_nocom, .tktr_com_nb{

	display:block;
	background: #F8FFD1;
	color: #3D3C21;
	border-bottom:1px solid #3D3C21;
	padding: 10px;
}
a .tktr_com_foot{
